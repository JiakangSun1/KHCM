...
class Dataset_maid_rgb(BaseDataset):
    # ORIGINAL SatMAE:
    ...
    def __getitem__(self, index):
        ...
        # #====================
        # single_image_name = os.path.splitext(single_image_name)[0] + '.pt'
        # #=====================
        single_image_name = (
            single_image_name
            if os.path.isabs(single_image_name)
            # else os.path.join(self.base_path+'\test', single_image_name)
            else 'train/'+single_image_name
        )
        # Open image
        img_as_img = Image.open(single_image_name)
        #=============================
        # img_as_img = torch.load(single_image_name).permute(2, 0, 1).float()
...
def build_fmow_dataset(is_train: bool, args) -> BaseDataset:
    ...
    elif args.dataset_type == "maid_rgb":
        mean = Dataset_maid_rgb.mean
        std = Dataset_maid_rgb.std
        transform = Dataset_maid_rgb.build_transform(
            is_train, args.input_size, mean, std
        )
        dataset = Dataset_maid_rgb(csv_path, transform)
