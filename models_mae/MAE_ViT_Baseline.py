...
class MAE_ViT_Baseline(MAE_ViT_Shared):
    ...
    def forward_encoder(self, x, mask_ratio, is_teacher=False):
        ...
        if self.use_xformers:
            ...
            for i, blk in enumerate(self.encoder):
                x = blk(x)
                if i in self.out_indices and is_teacher == False:
        ...
        if is_teacher == False:
            ...
            res = res.sum(dim=0)
            res = self.encoder_norm(res)
        # LayerNorm
        x = self.encoder_norm(x)
        if is_teacher == False:
            return x, mask, ids_restore, res
        return x
    ...
    def forward(self, imgs, mask_ratio=0.75,
                mask_seed=None, return_embeds=False, is_teacher=False):
        if mask_seed is not None:
            torch.manual_seed(mask_seed)
        if is_teacher:
            encoder_embed = self.forward_encoder(imgs, mask_ratio, is_teacher=is_teacher)
            return encoder_embed
        encoder_embed, mask, ids_restore, res = self.forward_encoder(imgs, mask_ratio, is_teacher=is_teacher)
        # decoder_pred, decoder_embed = self.forward_decoder(
        #     encoder_emb, ids_restore
        # )  # [N, L, p*p*3]
        decoder_pred, decoder_embed = self.forward_decoder(
            res, ids_restore
        )  # [N, L, p*p*3]
